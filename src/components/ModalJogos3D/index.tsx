...
import styles from './ModalJogos3D.module.css'

interface ModalJogos3DProps {
  isOpen: boolean
  onClose: () => void
}

export const ModalJogos3D: React.FC<ModalJogos3DProps> = ({ isOpen, onClose }) => {
  const t = useTranslations('modals.games')
  ...
  const handleSaibaMais = () => {
    ...
    setTimeout(() => {
      router.push(`/${locale}/servicos/desenvolvimento-jogos-3d`)
    }, 1400)
  }

  return (
    <>
      <InlineModal3D isOpen={isOpen} onClose={onClose} className={`${styles.jogosModal} ${isExpanding ? styles.expanding : ''}`}>
